...
class Constants:
    ...
    CLICK_AREA_DEFINITIONS: Dict[str, Dict[str, Any]] = {
        ...
        "14": {
            "name": "自動旋轉 10 次",
            "x_ratio": AUTO_SPIN_10_X_RATIO,
            "y_ratio": AUTO_SPIN_10_Y_RATIO,
        },
        "15": {
            "name": "自動旋轉 50 次",
            "x_ratio": AUTO_SPIN_50_X_RATIO,
            "y_ratio": AUTO_SPIN_50_Y_RATIO,
        },
        "16": {
            "name": "自動旋轉 100 次",
            "x_ratio": AUTO_SPIN_100_X_RATIO,
            "y_ratio": AUTO_SPIN_100_Y_RATIO,
        },
        "17": {
            "name": "金額顯示位置",
...
class ImageDetector:
    ...
    def capture_click_area_screenshot(
        ...
        try:
            ...
            # 轉換為 numpy array
            full_screenshot = np.array(screenshot_img)
            ...
            if is_success:
                with open(output_path, 'wb') as f:
                    f.write(buffer.tobytes())
                self.logger.info(f"已儲存點擊區域截圖: {output_path.name}")
                return True
            else:
                self.logger.error("截圖編碼失敗")
                return False

        except Exception as e:
            self.logger.error(f"擷取點擊區域截圖失敗 [{area_name}]: {e}")
            return False
    ...
    def get_current_betsize(
        ...
        與 capture_betsize_template 共用 _get_betsize_crop_region，
        但使用較大的邊距（margin_multiplier=2.0）以容納更大的搜尋空間。
        ...
        for attempt in range(retry_count):
            try:
                ...
                # 使用 _get_betsize_crop_region 計算裁切範圍
                # margin_multiplier=2.0 提供較大的搜尋範圍以容納模板匹配
                crop_region = self._get_betsize_crop_region(
                    driver, screenshot_img, margin_multiplier=2.0
                )
                if crop_region is None:
                    if not silent:
                        self.logger.error("無法計算金額區域裁切範圍")
                    continue
...
class GameControlCenter:
            ...
            try:
                ...
                for bt in active_browsers:
                    ...
                    if lobby_return_detected.get(browser_index, False):
                        self.logger.warning(
                            f"瀏覽器 {browser_index} ({username}) 偵測到返回大廳畫面，處理中..."
                        )
                        # 非同步啟動恢復執行緒
    ...
    def _handle_lobby_return_recovery(self, bt: 'BrowserThread') -> None:
        ...
        1. 點擊返回大廳按鈕（回到大廳）
        2. 導航到登入頁面 (_recovery_navigate_to_login)
        3. 重新進入遊戲（搜尋遊戲並進入）
        4. 執行圖片檢測流程（遊戲登入 → 遊戲開始/錯誤訊息）
    ...
    def _recovery_navigate_to_game(self, bt: 'BrowserThread') -> bool:
        ...
        for attempt in range(Constants.MAX_RETRY_ATTEMPTS):
            try:
                ...
                # 6. 恢復視窗大小（確保截圖與模板尺寸一致）
                try:
                    driver.set_window_size(Constants.WINDOW_WIDTH, Constants.WINDOW_HEIGHT)
                    time.sleep(Constants.NORMAL_WAIT)
                except Exception as e:
                    self.logger.debug(f"恢復視窗大小失敗: {e}")
